...
import StorefrontIcon from '@mui/icons-material/Storefront'; // Naya Icon Seller ke liye
...
const PrimaryDropDownMenu = ({ setTogglePrimaryDropDown, user }) => {
    ...
    return (
        <div className="absolute w-60 -left-24 ml-2 top-9 bg-white shadow-2xl rounded flex-col text-sm text-gray-800 z-50">
            ...
            {/* --- SELLER DASHBOARD (Sirf approved seller ko dikhega) --- */}
{user.role === "seller" && user.status === "approved" &&
    <Link className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50" to="/seller/dashboard">
        <span className="text-primary-blue"><StorefrontIcon sx={{ fontSize: "18px" }} /></span>
        Seller Dashboard
    </Link>
}

{/* --- PENDING STATUS (Agar form bhara hai lekin approve nahi hua) --- */}
{user.role === "seller" && user.status === "pending" &&
    <div className="pl-3 py-3.5 border-b flex gap-3 items-center bg-yellow-50 text-yellow-600">
        <span className="text-yellow-600"><StorefrontIcon sx={{ fontSize: "18px" }} /></span>
        Approval Pending...
    </div>
}

{/* --- BECOME A SELLER (Sirf normal user ko dikhega) --- */}
{user.role === "user" &&
    <Link className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50 bg-yellow-50" to="/become-seller">
        <span className="text-orange-500"><StorefrontIcon sx={{ fontSize: "18px" }} /></span>
        <span className="font-medium text-orange-600">Become a Seller</span>
    </Link>
}

            <Link className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50" to="/account">
