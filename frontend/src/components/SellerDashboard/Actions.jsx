...
const Actions = ({ id, deleteHandler, name, editRoute }) => {
    ...
    // Dynamic path banayein: agar seller hai toh /seller/, varna /admin/
    const baseRoute = user.role === "seller" ? "/seller" : "/admin";

    return (
        <>
            <div className="flex justify-between items-center gap-3">
                {editRoute !== "review" && (
                    /* Yahan link ko dynamic kar diya gaya hai */
                    <Link to={`${baseRoute}/${editRoute}/${id}`} className="text-blue-600 hover:bg-blue-200 p-1 rounded-full bg-blue-100">
                        <EditIcon />
                    </Link>
                )}
            ...
            >
                ...
                <DialogActions>
                    ...
                    <button onClick={() => { deleteHandler(id); handleClose(); }} className="py-2 px-6 ml-4 rounded bg-red-600 hover:bg-red-700 text-white shadow">Delete</button>
                </DialogActions>
            </Dialog>
        </>
    );
