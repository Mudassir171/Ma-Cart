...
{(() => {
  // Filter products that have both a discount and an active flash sale timer
  const validFlashSaleProducts = discountedProducts && discountedProducts.length > 0 
    ? discountedProducts.filter(item => item.cuttedPrice > item.price && item.saleTill) 
    : [];

  return validFlashSaleProducts.length > 0 && (
