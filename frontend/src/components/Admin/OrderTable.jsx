...
const OrderTable = () => {
    ...
    useEffect(() => {
        ...
        dispatch({ type: "UPDATE_ORDER_RESET" }); // Reset flag
        dispatch(getAllOrders()); // Table refresh karein
     }
    ...
    const columns = [
        ...
        },
        {
            ...
            flex: 0.2,
            renderCell: (params) => (
                <span className="font-medium uppercase text-gray-600 text-xs">{params.row.method}</span>
            )
        },
        {
            field: "receipt",
        ...
        { field: "itemsQty", headerName: "Items Qty", type: "number", minWidth: 100, flex: 0.1 },
        {
    field: "amount",
    headerName: "Amount",
    type: "number",
    ...
    flex: 0.2,
    renderCell: (params) => (
        // Optional chaining (?.) aur fallback (|| 0) use karein
        <span className="font-semibold text-gray-700">
            ₹{(params.row.price * params.row.quantity || 0).toLocaleString()}
        </span>
    ),
},
      {
    ...
    renderCell: (params) => (
        <span className="font-semibold text-green-700">
            ₹{(params.row.commission || 0).toLocaleString()}
        </span>
    ),
},
        { field: "orderOn", headerName: "Order On", minWidth: 150, flex: 0.4 },
       {
    field: "actions",
    headerName: "Actions",
    minWidth: 150,
    ...
    renderCell: (params) => (
        <Actions 
            ...
            isOrder={true} // Yeh zaroori hai taaki dropdown dikhe
            currentStatus={params.row.status} // Current status dikhane ke liye
            updateStatusHandler={(orderId, itemId, status) => {
                dispatch(updateOrder(orderId, { itemId, status }));
            }}
        />
    ),
},
    ...
    orders && orders.forEach((order) => {
        ...
        order.orderItems.forEach((item) => {
            rows.unshift({
                id: item._id, // Item ka unique ID
                orderId: order._id, // Parent order ka ID
                name: item.name,
                status: item.status, // Item-specific status
                price: item.price,
                quantity: item.quantity,
                commission: item.adminCommission,
                orderOn: formatDate(order.createdAt),
                method: order.paymentInfo?.method || "N/A",
                receipt: order.paymentInfo?.screenshot?.url || null,
            });
        });
    });
    return (
        <>
            <MetaData title="Admin Orders | Ma-Cart" />
