...
// 1. Stripe Payment Process Karein
exports.processPayment = asyncErrorHandler(async (req, res, next) => {
    // Amount ko check karein
    if (!req.body.amount) {
        return next(new ErrorHandler("Please provide payment amount", 400));
    }
...
// 2. Stripe API Key Frontend ko bhejein
exports.sendStripeApiKey = asyncErrorHandler(async (req, res, next) => {
    res.status(200).json({ 
